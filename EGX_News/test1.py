...
def Google_Sheet_Undate():
    ...
    with open('output.txt', 'r', encoding='utf-8') as textfile:
               ...
               for row in rows:         
                    ...
                    bulletin_number  =  (row.split(','))[0]              # Split line into bulletin number and PDF file names
                    Date_and_Time = (row.split(','))[1]
                    ...
                    for pdf_file in pdf_files:                           # -------------  get the links for all files found for each row  ------------
                        ...
                        while retries < max_retries:
                            try:
                                ...
                                # file_links.append(f'=HYPERLINK("{file_link}","Link{i}")')
                                # file_links.append(f'{file_link}')
                                # row.append(file_links)
