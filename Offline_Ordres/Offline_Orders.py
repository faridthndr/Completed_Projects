...
def Query_Google_Sync():
 ...
 #------------------------------------------------------ google sheet ----------------------------------------------------
    ...
    for i, row in enumerate(sheet1_row):               # the main loop that excute the boobking steps-----------------------
        ...
        if row[CustodianCode_index] !='' and row[Thndr_Code_Index] != '' and row[Price_Index] != '' and row[QTY_index] !=''  and row[ISIN_Code_index] != ''and row[Market_Code_index] != '' and row[Order_Type_Index] != '' and row[Comment_indx] != 'Comment' and row[Order_Type_Index]=='Sell':
