...
def Query_Google_Sync():
 ...
 #------------------------------------------------------ google sheet ----------------------------------------------------
    ...
    QTY_index = sheet1_header.index('OrgQty')
    T_Type_Index = sheet1_header.index('T-Type')

    
    Result_index = sheet1_header.index('Action')
    ...
    for i in range(First_Row - 1 , Last_Row ):                                                   # the main loop that excute the boobking steps-----------------------
        ...
        if sheet1_row[i][Result_index] == '' and sheet1_row[i][UnifiedCode1_index] !='' and sheet1_row[i][SymbolCode_index] != '' and sheet1_row[i][QTY_index] != '' and sheet1_row[i][T_Type_Index] == 'Sell' :
