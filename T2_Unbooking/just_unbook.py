...
def Query_Google_Sync():
 ...
 #------------------------------------------------------ google sheet ----------------------------------------------------
    ...
    if os.path.exists('Sheet_Url.txt'): 
        ...
        sheet = client.open_by_url(sheet_url).worksheet('Input_UniCode')
    else:
    ...
    Sheet3 = client.open_by_url(sheet_url).worksheet('code')
    ...
    for i, row in enumerate(sheet1_row):               # the main loop that excute the boobking steps-----------------------
        ...
        if row[Action1_index] == '' and i>0:             # Skip sheet1_header row
            last_row2 = len(Sheet2.col_values(1)) + 1
...
if __name__ == "__main__":
    ...
    ui.Server_input.setText("10.70.4.96")     #put value in server IP
