...
def send_slack_notification(value):  
    ...
    payload = {  
        "text": f"Warning: The number of rejection messages in the last inquiry has exceeded the safe limit and has reached: {formatted_value}"  
    }  
...
#-------------------------------------------------------------------------------------AZURE------------------------- 
    ...
    try:    
        ...
        # pass
        pyautogui.alert(f"Error: {q}")


    try:                                                               #   tray to add alert_results to google sheet
        alert_results = [list(row) for row in alert_results] 
        alert_results[0][0] = alert_results[0][0].strftime('%Y-%m-%d %H:%M:%S')
        trigger_value = Alert_Sheet.cell(2, Trigger_Index+1).value  
        Trigger_Num = int(alert_results[0][-1])

        if int(Trigger_Num)>= 0:
            current_values = Alert_Sheet.get('A2:D') 
            new_values = alert_results + current_values
            Alert_Sheet.update(values= new_values, range_name='A2:D')


            if (Trigger_Num) >= int(trigger_value):

                error = send_slack_notification(Trigger_Num)
                if error ==11:
                 ui.Status_label.setText("Can't send Slack Alert")
                 QApplication.processEvents()
    except Exception as d:
        pyautogui.alert(f"Error: {d}")

        error = 5
        return error   
    # error = send_slack_notification(1520)    #------------------
    # if error ==11:
         # ui.Status_label.setText("Can't send Slack Alert")
         # QApplication.processEvents()
    ...
    for i, row in enumerate(google_rows):               # the main loop that excute the boobking steps-----------------------
        ...
        if row[Action_index] == '' and row[Eligibility_index] == '1'  :          #   and i > 1430   and i > ???           
