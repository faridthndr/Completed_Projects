    ...
    if not WinActivate(BK_win,4):
       ...
       return error
        ...
        for _ in range(2):     
            ...
            try:
                x,y = pyautogui.locateCenterOnScreen(path+'Done.png',confidence = 0.8)                  # 90% matching the Img
                
            except Exception as e:
                time.sleep(0.1)
                
            else:
                pyautogui.leftClick(x,y, interval=0.001)
                comment = 'Not Reg'
                return comment


        x,y,img = Detect_img ('check.png',0,0,50)
        if img ==1 :
            time.sleep(0.1)
            pyautogui.leftClick(x,y-20 , interval=0.001)
...
def Query_Google_Sync():
 ...
 #------------------------------------------------------ google sheet ----------------------------------------------------
    ...
    if os.path.exists('Sheet_Url.txt'): 
        ...
        sheet = client.open_by_url(sheet_url).worksheet('UnifiedCode')
    else:
